    ...
    .forEach(
        (line) => {
            ...
            if (header === 'seeds' && seeds.length == 0){
                let seedNums = line.match(seedNumPattern).at(0).split(" ").map((s) => parseInt(s));

                for (let i = 0; i < seedNums.length-1; i+=2){
                    let r = range(seedNums[i], seedNums[i+1])
                    seeds = seeds.concat(r);
                }
            }
            else {
...
let minLocn = Math.min(...seedValues.map(m => m.seedLocn))
console.log(minLocn)